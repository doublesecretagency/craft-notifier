...
/**
 * Structural tests for the DigitalProductLicenseEvents helper.
 *
 * License events are the buyer-facing trigger for Digital Products work -
 * a license-granted notification ships the customer their key + product
 * info. A silent break here would drop the most user-visible Digital
 * Products notification path.
 */
class DigitalProductLicenseEventsTest extends TestCase
{
    ...
    {
        $path = dirname(__DIR__, 2) . '/src/helpers/events/DigitalProductLicenseEvents.php';
        $this->assertTrue(file_exists($path), "DigitalProductLicenseEvents helper should exist at: $path");
        $this->helperSource = file_get_contents($path);
        $this->reflection = new ReflectionClass(DigitalProductLicenseEvents::class);
    }
    ...
    public function testImportsLicenseAndEvent(): void
    {
        $this->assertStringContainsString('use ' . License::class, $this->helperSource);
    ...
    public function testAfterSaveGuardsAgainstNonLicenseSender(): void
    {
        $this->assertStringContainsString(
            '!($event->sender instanceof License)',
            $this->helperSource
        );
    }
    ...
    {
        $this->assertStringContainsString("'eventType' => 'digital-products-licenses'", $this->helperSource);
    ...
    public function testAllThreeMethodsDelegateToMessagesService(): void
    {
        $this->assertSame(
            ...
            'All three License helpers must delegate to messages->sendAll(...)'
        );
    }
